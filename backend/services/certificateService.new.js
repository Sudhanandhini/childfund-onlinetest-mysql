// backend/services/certificateService.js (PDF version)
  ...
  async generateCertificate(userData, submissionData) {
    ...
    return await new Promise((resolve, reject) => {
      try {
        ...
        doc.moveDown(0.5);
        doc.fillColor('#424242').font('Times-Roman').fontSize(14).text('42 years with children, in every stride', { align: 'center' });
        ...
        doc.moveDown(1);
        doc.font('Times-Bold').fontSize(20).fillColor('#2E7D32').text(`Score: ${totalScore}/${maxScore} (${isFinite(percentage) ? percentage.toFixed(1) : '0.0'}%)`, { align: 'center' });
        doc.moveDown(0.5);
        ...
        const leftX = doc.page.margins.left + pageWidth * 0.15;
        const rightX = doc.page.margins.left + pageWidth * 0.65;

        doc.fontSize(12).fillColor('#000').text('____________________', leftX, y);
        doc.text('Authorized Signatory', leftX, y + 15);

        doc.fontSize(12).text('____________________', rightX, y);
        doc.text('Program Lead', rightX, y + 15);
